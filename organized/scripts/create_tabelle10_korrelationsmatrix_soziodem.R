...
# ================================================================================
# TABELLE 10: PEARSON-KORRELATIONSMATRIX DER ZIELVARIABLEN MIT SOZIODEMOGRAFISCHEN UND NUTZUNGSBEZOGENEN VARIABLEN
# ================================================================================

cat("================================================================================\n")
cat("TABELLE 10: PEARSON-KORRELATIONSMATRIX MIT SOZIODEMOGRAFISCHEN VARIABLEN\n")
cat("================================================================================\n")
...
if(length(available_target_vars) > 0) {
  cat("✓ Verfügbare Zielvariablen:", paste(available_target_vars, collapse = ", "), "\n")
} else {
  cat("❌ Keine Zielvariablen verfügbar!\n")
  stop("Keine Zielvariablen berechnet")
}

if(length(available_socio_vars) > 0) {
  cat("✓ Verfügbare soziodemografische Variablen:", paste(available_socio_vars, collapse = ", "), "\n")
} else {
  cat("❌ Keine soziodemografischen Variablen verfügbar!\n")
  stop("Keine soziodemografischen Variablen verfügbar")
}
...
for(var in available_target_vars) {
  ...
  cor_table_data[[names(cor_values)[1]]] <- marked_values[1]
  if(length(marked_values) > 1) {
    for(k in 2:length(marked_values)) {
      cor_table_data[[names(cor_values)[k]]] <- marked_values[k]
    }
  }
}
...
# Dynamische Spaltenbreiten für soziodemografische Variablen
for(var in available_socio_vars) {
  tabelle10 <- tabelle10 %>% cols_width(!!var ~ px(100))
}
...
# HTML exportieren
cat("\n=== TABELLE EXPORTIEREN ===\n")
html_file <- "organized/images/clustering/tabelle10_korrelationsmatrix_soziodem.html"
...
cat("\n================================================================================\n")
cat("TABELLE 10: KORRELATIONSMATRIX MIT SOZIODEMOGRAFISCHEN VARIABLEN ERSTELLT\n")
cat("================================================================================\n")
