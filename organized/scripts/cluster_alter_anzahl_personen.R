...
# Balkendiagramm: Anzahl Personen pro Altersgruppe und Cluster
...
cat("=== BALKENDIAGRAMM: ANZAHL PERSONEN PRO ALTERSGRUPPE ===\n")
...
ki_alter_count <- ki_results$data %>%
  group_by(Cluster_Name, Alter_Kat) %>%
  summarise(Anzahl = n(), .groups = 'drop') %>%
  filter(Alter_Kat %in% c("18-25", "26-35", "36-45", "46-55")) # Nur die gewünschten Altersgruppen

cat("\nKI-Gruppe - Anzahl Personen pro Altersgruppe und Cluster:\n")
print(ki_alter_count)

# Balkendiagramm erstellen
p_ki_alter_count <- ggplot(ki_alter_count, aes(x = Cluster_Name, y = Anzahl, fill = Alter_Kat)) +
...
# PNG speichern
ggsave("../output/images/cluster_alter_anzahl_personen.png", p_ki_alter_count, 
       width = 12, height = 8, dpi = 300, bg = "white")

cat("PNG gespeichert: ../output/images/cluster_alter_anzahl_personen.png\n")
...
cat("\nAltersverteilung in KI-Gruppe:\n")
print(table(ki_results$data$Alter_Kat))
