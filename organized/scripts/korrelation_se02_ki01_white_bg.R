...
# Tabelle mit weißem Hintergrund für Korrelationsanalyse: SE02 vs. KI01

library(dplyr)
library(flextable)

cat("=== TABELLE MIT WEISSEM HINTERGRUND KORRELATIONSANALYSE: SE02 VS. KI01 ===\n")
...
# Flextable mit weißem Hintergrund erstellen
ft <- flextable(table_data) %>%
  ...
  border_outer(border = officer::fp_border(color = "black", width = 0.5)) %>%
  border_inner_h(border = officer::fp_border(color = "black", width = 0.25)) %>%
  autofit()

# PNG mit weißem Hintergrund speichern
save_as_image(ft, path = "../output/images/korrelation_se02_ki01_tabelle.png", 
              width = 8, height = 2, res = 300, bg = "white")
...
cat("\n=== TABELLE MIT WEISSEM HINTERGRUND ERFOLGREICH ERSTELLT ===\n") 