...
# ================================================================================
# TABELLE B5: MITTELWERTE DER SKALA "IDENTIFIKATION" AUF ITEM-EBENE
# ================================================================================

cat("================================================================================\n")
cat("TABELLE B5: MITTELWERTE DER SKALA \"IDENTIFIKATION\" AUF ITEM-EBENE\n")
cat("================================================================================\n")
...
# ID Items identifizieren
cat("\n=== ID ITEMS IDENTIFIZIEREN ===\n")
id_items <- c("ID01_01", "ID01_02", "ID01_03", "ID01_04")

# Verfügbare Items prüfen
available_id_items <- id_items[id_items %in% names(data)]
cat("✓ Verfügbare ID Items:", paste(available_id_items, collapse = ", "), "\n")

if(length(available_id_items) == 0) {
  cat("❌ Keine ID Items gefunden\n")
  stop("Keine ID Items gefunden")
}
...
for(item in available_id_items) {
...
item_names <- c("ID01", "ID02", "ID03", "ID04")
...
tabelle_b5 <- gt(table_data) %>%
  # Titel und Untertitel
  tab_header(
    title = "Tabelle B5",
    subtitle = "Mittelwerte der Skala \"Identifikation\" auf Item-Ebene"
  ) %>%
  # Spaltenlabels
  cols_label(
    Gruppe = "Gruppe",
  ...
  cols_align(
    align = "center",
    columns = c("n", "ID01", "ID02", "ID03", "ID04")
  ) %>%
  # APA7: Tabellenformat
  tab_options(
...
cat("• Jede Spalte zeigt ein einzelnes Item der ID-Skala\n")
cat("• n = Anzahl der gültigen Antworten pro Gruppe\n")

cat("\n🔍 IHRE ERGEBNISSE:\n")
cat("Skala: Identifikation (ID)\n")
cat("Items: ID01 bis ID04\n")
cat("Gruppen: KI-Avatar vs. Mensch\n")

# Wichtige Befunde hervorheben
cat("\n📈 WICHTIGE BEFUNDE:\n")
for(i in 1:length(available_id_items)) {
  item <- available_id_items[i]
  item_short <- paste0("ID0", i)
...
cat("\n================================================================================\n")
cat("TABELLE B5: MITTELWERTE AUF ITEM-EBENE ERSTELLT\n")
cat("================================================================================\n")

cat("\n📋 ZUSAMMENFASSUNG:\n")
cat("• Tabelle B5 mit Item-Mittelwerten der ID-Skala erstellt\n")
cat("• Separate Werte für KI-Avatar und Mensch Gruppe\n")
cat("• Alle 4 Items der ID-Skala berücksichtigt\n")
